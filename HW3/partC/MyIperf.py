...
def set_router_tc_attr(net: Mininet):
    # Buffer Size, set to 10Kb, 5Mb or 25Mb in every run
    buffer_size = '10kb'
    # Set the bandwidth to 100mb
    bandwidth = '100mbit'
    # delay to be introduced in each router interface
    delay = '30ms'

    router_intf_map = {R1: [r1_eth0, r1_eth1, r1_eth2], R2: [r2_eth0, r2_eth1], R3: [r3_eth0, r3_eth1], R4: [r4_eth0, r4_eth1, r4_eth2]}
    
    for router, intf_list in router_intf_map.items():
        for intf in intf_list:
            # Limit bandwidth and buffer size of every interface of all routers with the tc tbf option
            net[router].cmd('tc qdisc add dev {} root handle 1: tbf rate {} buffer {} limit {}'.format(intf, bandwidth, buffer_size, buffer_size))
            # Introduce delay in every interface of all routers with the tc netem option
            net[router].cmd('tc qdisc add dev {} parent 1:1 handle 10: netem delay {}'.format(intf, delay))
...
def run():
    ...
    # Add some delay to allow all routers to broadcast their IPs to other routers
    time.sleep(1)
    # Pingall result
